...
fn print_process(target: &String) -> Option<bool> {
    let process = get_target(target).ok()??;
    process.print();

    let children = ps_utils::get_child_processes(process.pid).ok();
    if children.is_none() {
        return Some(true);
    }
